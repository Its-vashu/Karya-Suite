...
import { X, Award, Mail, CheckCircle, AlertCircle,Send } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.award_type) {
        throw new Error('Please select an award type');
      }
      if (!formData.badge_level) {
        throw new Error('Please select a badge level');
      }
      if (!formData.appreciation_message || formData.appreciation_message.trim() === '') {
        throw new Error('Please enter an appreciation message');
      }
      if (!formData.month) {
      ...
      const appreciationData = {
        ...
        appreciation_message: formData.appreciation_message.trim(),
        month: formData.month, // Using the month name directly from form state
        year: parseInt(formData.year)
      };
      
      // Log the data being sent
      //console.log('Sending appreciation data:', appreciationData);
      
      const appreciationResponse = await APPRECIATION_API.createAppreciation(appreciationData);
      //console.log('Appreciation created successfully:', appreciationResponse);
      ...
      if (formData.send_email) {
        ...
        try {
          ...
          if (!selectedEmployee) {
            throw new Error(`Employee with ID ${formData.employee_id} not found`);
          }
          ...
          //console.log('Sending appreciation email:', emailData);
          await APPRECIATION_API.sendAppreciationEmail(emailData);
          //console.log('Appreciation email sent successfully');
        } catch (emailError) {
          //console.error('Email sending failed:', emailError);
          // Show a warning but don't fail the entire process if email fails
          alert(`Appreciation was created successfully, but there was a problem sending the email: ${emailError.message || 'Unknown error'}`);
        } finally {
      ...
      toast.success('Appreciation has been successfully created!');
      ...
      const errorMessage = err.response?.data?.detail || err.message || 'Failed to create appreciation';
